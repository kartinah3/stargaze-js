...
export const VersionInfo = {
  ...
  decode(input: Reader | Uint8Array, length?: number): VersionInfo {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = globalThis.Object.create(baseVersionInfo) as VersionInfo;
    message.buildDeps = [];
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
...
export const Module = {
  ...
  decode(input: Reader | Uint8Array, length?: number): Module {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = globalThis.Object.create(baseModule) as Module;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
